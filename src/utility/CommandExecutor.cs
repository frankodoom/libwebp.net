       ...
       public static async Task<FileStream> Execute(string Command)
        {
            ...
            if (OperatingSystem.IsWindows())
            {
                ...
                {
                    throw new CommandExecutionException("LibWebP.Net CommandExecutor Failed: " + ex.Message, ex.InnerException);
                }
            }

            if (OperatingSystem.IsLinux())
            {
                using var stream = Assembly.GetExecutingAssembly().GetManifestResourceStream("Libwebp.Net.codecs.linux.cwebp.sh");
                //copy codec to memory
                using var ms = new MemoryStream();
                stream.CopyTo(ms);
                ...
                //write codec to temp location on server if file does not exist when FIleMode is set to OpenOrCreate
                using var fs = new FileStream(path0 + "cwebp.sh", FileMode.OpenOrCreate, FileAccess.Write, FileShare.None);
                ms.WriteTo(fs);
                fs.Close();
                ...
                {
                    throw new CommandExecutionException("LibWebP.Net CommandExecutor Shell Failed: " + ex.Message, ex.InnerException);
                }
            }
